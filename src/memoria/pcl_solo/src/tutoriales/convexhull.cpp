...
void setVisualizationType(boost::shared_ptr<pcl::visualization::PCLVisualizer> &viewer, const string nombre, int vis){
    /*
    0: points
    1: wireframe
    2: surface
...
void drawPoint(boost::shared_ptr<pcl::visualization::PCLVisualizer> &viewer, pcl::PointXYZ p, const string nombre, float r, float g, float b){
    viewer->addSphere(p, 0.1, r, g, b, nombre, 0);
}

void drawPolygon(boost::shared_ptr<pcl::visualization::PCLVisualizer> &viewer,pcl::PolygonMesh mesh, pcl::Vertices poly, float r, float g, float b){
    ...
    setVisualizationType(viewer, nombre, 2);
}
//--------- END VISUALIZADOR -----------
...
void biggestPolygon(pcl::PolygonMesh mesh, pcl::Vertices &polygon, Eigen::Vector3f &normal){
...
pcl::PointXYZ getMassCenter(pcl::PolygonMesh mesh){
...
int main(int argc, char **argv){
    ...
    // Crear visualizador
    boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer = meshVis();
    ...
    biggestPolygon(hull, biggest, b_normal);

    printf("Normal: (%f, %f, %f)\n", b_normal[0], b_normal[1], b_normal[2]);

    // Visualizar polígono
    drawPolygon(viewer, hull, biggest, 1.0, 0.0, 0.0);
    // Obtener centro de masa
    pcl::PointXYZ cm = getMassCenter(hull);
    // Mostrar centro de masa
    drawPoint(viewer, cm, "Centro de Masa", 0.0, 1.0, 0.0);
    // Proyectar centro de masa sobre plano del polígono
    pcl::PointXYZ cm_projected = projectPointOverPolygon(cm, b_normal, biggest, hull);
    // Mostrar proyección del centro de masa
    drawPoint(viewer, cm_projected, "cm proyectado", 0.0, 0.0, 1.0);
    // La proyección del centro de masa está dentro del polígono?
    bool in_polygon = pointInPolygon(cm_projected, biggest, hull);
    printf("El punto %s se proyecta. %s\n", (in_polygon ? "SI":"NO"), (in_polygon ? "EXITO":"FRACASO"));
