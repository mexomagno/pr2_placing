...
Viewer::Viewer(float r, float g, float b, float axisscale){
    ...
    viewer_->addCoordinateSystem(axisscale);
...
void Viewer::drawBox(const string shape_id, float min_x, float max_x, float min_y, float max_y, float min_z, float max_z, float r, float g, float b, float a){
    viewer_->addCube(min_x, max_x, min_y, max_y, min_z, max_z, r, g, b, shape_id, 0);
    viewer_->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_OPACITY, a, shape_id);
}
